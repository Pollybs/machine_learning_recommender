...
def extract_ingredients(x):
    if x != "error":
        ingredient_list = [ingredient.strip() for ingredient in x.split('\n')]
...
def extract_prefix(x):
    prefix = "Time\nCooks In"
    if isinstance(x, str) and x.startswith("Time\nCooks In"):
        return x[len(prefix):].strip()
    else:
        return x
    
#jp_df["time_cook2"] = jp_df["time_cook"].apply(extract_time)
    
replacements = {
    'mexican, tex-mex': 'mexican',
    'tex-mex, mexican': 'mexican',
    'tex-mex': 'mexican',
    'vegan': '',
    'american-italian':  'american',
    'indian, gluten free' : 'indian',             
    'gluten free, asian': 'asian'             
}


#simpleveg_urls['category'] = simpleveg_urls['category'].replace(replacements, regex=True)
